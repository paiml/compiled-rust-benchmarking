...
//!
//! Workload Type: Memory-bound (tree traversal, sequential access)
...
/// Benchmark BTreeMap insert and lookup operations
fn benchmark_btreemap(size: usize) -> i64 {
...
mod tests {
    ...
    fn test_btreemap_empty() {
        assert_eq!(benchmark_btreemap(0), 0);
    }

    #[test]
    fn test_btreemap_small() {
        // 0*2 + 1*2 + 2*2 + 3*2 + 4*2 = 0 + 2 + 4 + 6 + 8 = 20
        assert_eq!(benchmark_btreemap(5), 20);
    }

    #[test]
    fn test_btreemap_medium() {
        // Sum of 2*i for i from 0 to 99 = 2 * (0+1+...+99) = 2 * 4950 = 9900
        assert_eq!(benchmark_btreemap(100), 9900);
    }

    #[test]
    fn test_btreemap_target_workload() {
        // Sum of 2*i for i from 0 to 999,999
        // = 2 * (0+1+...+999999) = 2 * n(n-1)/2 = n(n-1)
        // = 1000000 * 999999 = 999,999,000,000
        assert_eq!(benchmark_btreemap(1_000_000), 999_999_000_000);
    }
}
